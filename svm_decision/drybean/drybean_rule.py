...
preds_12 = [1 if x > upper_bound or x < lower_bound else 0 for x in test_data['ShapeFactor1']]
"""
Z-分数法
"""
test_data = drybean.iloc[idx_test]
...
preds_13 = [1 if x > 3 or x < -3 else 0 for x in test_data['ShapeFactor1_zscore']]
...
dataframe.to_csv("drybean_rule.csv",index=False,sep=',')
