...
# 两个简单统计方法中都选择Height列作为计算源
"""
四分位极差法
# """
# test_data = balita.iloc[idx_test]
# q1 = test_data['Height'].quantile(0.25)
# q3 = test_data['Height'].quantile(0.75)
# iqr = q3 - q1

# lower_bound = q1 - 1.5 * iqr
# upper_bound = q3 + 1.5 * iqr

# outliers_quartile = test_data[(test_data['Nutrition_Status'] == target_class) & ((test_data['Height'] < lower_bound) | (test_data['Height'] > upper_bound))]
...
test_data['Height_zscore'] = stats.zscore(test_data['Height'])
outliers_Z_score = test_data[(test_data['Height_zscore'] > 3) | (test_data['Height_zscore'] < -3)]

# outliers_Z_score = test_data[((test_data['Height_zscore'] > 3) | (test_data['Height_zscore'] < -3)) & (test_data['Nutrition_Status'] == target_class)]
print(outliers_Z_score)
outliers_index = outliers_Z_score.index

print(len(outliers_index))
# print("测试集中指定类异常值索引为：", outliers_index)
test_class_num = len(test_data[test_data['Nutrition_Status'] == target_class])
