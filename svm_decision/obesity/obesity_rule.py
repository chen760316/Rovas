...
四分位极差法
"""
test_data = obesity.iloc[idx_test]
q1 = test_data['BMI'].quantile(0.25)
q3 = test_data['BMI'].quantile(0.75)
iqr = q3 - q1

lower_bound = q1 - 1.5 * iqr
upper_bound = q3 + 1.5 * iqr

preds_12 = [1 if x > upper_bound or x < lower_bound else 0 for x in test_data['BMI']]
"""
Z-分数法
"""
test_data = obesity.iloc[idx_test]
# test_data = test_data[test_data['Nutrition_Status'] == target_class]
test_data['BMI_zscore'] = stats.zscore(test_data['BMI'])
preds_13 = [1 if x > 3 or x < -3 else 0 for x in test_data['BMI_zscore']]
...
dataframe.to_csv("obesity_rule.csv",index=False,sep=',')
