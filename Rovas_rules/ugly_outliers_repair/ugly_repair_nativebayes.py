...
predict_fn = lambda x: nb_classifier.predict_proba(x)
...
# 预测每个样本被划分到的类别
predicted_labels = nb_classifier.predict(X_copy)
...
# subsection 原始数据集上训练的nb模型在训练集和测试集中分错的样本比例
...
print("训练样本中被nb模型错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))

# 测试样本中被nb模型错误分类的样本
wrong_classified_test_indices = np.where(y_test != test_label_pred)[0]
print("测试样本中被nb模型错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))

# 整体数据集D中被nb模型错误分类的样本
print("完整数据集D中被nb模型错误分类的样本占总完整数据的比例：",
      (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))/(len(y_train) + len(y_test)))

# subsection 加噪数据集上训练的nb模型在训练集和测试集中分错的样本比例

print("*" * 100)
train_label_pred_noise = nb_classifier.predict(X_train_copy)
test_label_pred_noise = nb_classifier.predict(X_test_copy)

# 加噪训练样本中被nb模型错误分类的样本
wrong_classified_train_indices_noise = np.where(y_train != train_label_pred_noise)[0]
print("加噪训练样本中被nb模型错误分类的样本占总加噪训练样本的比例：",
...
nb.fit(X_copy_inners, y_inners)

# 预测异常值
y_pred = nb.predict(X_copy_repair)
...
# # 训练样本中被nb模型错误分类的样本
# wrong_classified_train_indices = np.where(y_train != y_train_pred)[0]
# print("加噪标签修复后，训练样本中被nb模型错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))
#
# # 测试样本中被nb模型错误分类的样本
# wrong_classified_test_indices = np.where(y_test != y_test_pred)[0]
# print("加噪标签修复后，测试样本中被nb模型错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))
#
# # 整体数据集D中被nb模型错误分类的样本
# print("加噪标签修复后，完整数据集D中被nb模型错误分类的样本占总完整数据的比例：",
#       (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))/(len(y_train) + len(y_test)))