...
wrong_classified_train_indices = np.where(y_train != decision_tree_model.predict(X_train))[0]
print("训练样本中被decision_tree模型错误分类的样本占总训练样本的比例：",
      len(wrong_classified_train_indices)/len(y_train))

# 测试样本中被decision_tree模型错误分类的样本
wrong_classified_test_indices = np.where(y_test != decision_tree_model.predict(X_test))[0]
print("测试样本中被decision_tree模型错误分类的样本占总测试样本的比例：",
...
# 获取最佳模型
decision_tree_model_noise = grid_search.best_estimator_
train_label_pred_noise = decision_tree_model_noise.predict(X_train_copy)

# 加噪训练样本中被decision_tree模型错误分类的样本
wrong_classified_train_indices_noise = np.where(y_train != decision_tree_model_noise.predict(X_train_copy))[0]
...
wrong_classified_test_indices_noise = np.where(y_test != decision_tree_model_noise.predict(X_test_copy))[0]
...
predict_fn = lambda x: decision_tree_model_noise.predict_proba(x)
...
# 在加噪数据集D'上训练的decision_tree模型，其分类错误的样本在原含噪数据D'中的索引
