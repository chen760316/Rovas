...
predict_fn = lambda x: nb_model_noise.predict_proba(x)
...
# 在加噪数据集D'上训练的nb模型，其分类错误的样本在原含噪数据D'中的索引
