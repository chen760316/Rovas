...
# SUBSECTION wine-quality数据集 (randomforest 模型下分类效果不好)
...
rf_classifier.fit(X_train_copy, y_train)
...
# 预测每个样本被划分到的类别
predicted_labels = rf_classifier.predict(X_copy)
...
print("*" * 100)
# rf_clf = RandomForestClassifier(n_estimators=50, max_depth=10, min_samples_split=10, min_samples_leaf=4, random_state=42)
...
print("训练样本中被rf模型错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))

# 测试样本中被rf模型错误分类的样本
wrong_classified_test_indices = np.where(y_test != test_label_pred)[0]
print("测试样本中被rf模型错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))
...
print("*" * 100)
train_label_pred_noise = rf_classifier.predict(X_train_copy)
test_label_pred_noise = rf_classifier.predict(X_test_copy)
...
# section 方案一：对X_copy中需要修复的元组进行标签修复（rf方法）
...
# rf.fit(X_copy_inners, y_inners)
#
# # 预测异常值
# y_pred = rf.predict(X_copy_repair)
#
# # 替换异常值
...
# rf_repair = RandomForestClassifier(n_estimators=50, max_depth=10, min_samples_split=10, min_samples_leaf=4, random_state=42)
rf_repair = RandomForestClassifier()
rf_repair.fit(X_train_copy, y_train)
y_train_pred = rf_repair.predict(X_train_copy)
y_test_pred = rf_repair.predict(X_test_copy)
...
# 训练样本中被rf模型错误分类的样本
wrong_classified_train_indices = np.where(y_train != y_train_pred)[0]
print("加噪标签修复后，训练样本中被rf模型错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))

# 测试样本中被rf模型错误分类的样本
wrong_classified_test_indices = np.where(y_test != y_test_pred)[0]
print("加噪标签修复后，测试样本中被rf模型错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))
