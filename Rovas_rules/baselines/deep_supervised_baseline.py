...
least_frequent_labels = [label for label, count in label_counts.items() if count == min_count]

# 找到对应的元组索引
indices_least_frequent_labels = []
for label in least_frequent_labels:
    indices = np.where(y_train == label)[0]
    indices_least_frequent_labels.extend(indices)
# 转换为 numpy 数组（可选）
anom_id = np.array(indices_least_frequent_labels)
...
out_clf = PReNet(epochs=epochs,
                  ...
                  lr=lr)
known_anom_id = np.random.choice(anom_id, 100, replace=False)
y_semi = np.zeros_like(y_train, dtype=int)
y_semi[known_anom_id] = 1
out_clf.fit(X_train, y_semi)
...
# section 方案三：对X_copy中需要修复的元组借助knn进行修复
...
# section 方案五：仅将训练集中需要修复的元组直接删除并在去除后的训练集上训练svm模型，测试集保持不变
...
# # 训练样本中被SVM模型错误分类的样本
# wrong_classified_train_indices = np.where(y_train_copy_repair != y_train_pred)[0]
# print("删除需要修复的训练样本后，训练样本中被SVM模型错误分类的样本占总训练样本的比例：",
#       len(wrong_classified_train_indices)/len(y_train_copy_repair))
#
# # 测试样本中被SVM模型错误分类的样本
# wrong_classified_test_indices = np.where(y_test != y_test_pred)[0]
# print("删除需要修复的训练样本后，测试样本中被SVM模型错误分类的样本占总测试样本的比例：",
#       len(wrong_classified_test_indices)/len(y_test))
#
# # 整体数据集D中被SVM模型错误分类的样本
# print("删除需要修复的训练样本后，完整数据集D中被SVM模型错误分类的样本占总完整数据的比例：",
#       (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))
#       /(len(y_train_copy_repair) + len(y_test)))

# section 方案六：仅将训练集中需要修复的元组直接删除并在去除后的训练集上训练svm模型
#  测试训练集和测试集的分类准确度时，训练集和测试集保持不变
