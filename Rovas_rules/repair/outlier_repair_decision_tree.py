...
# SECTION CART决策树模型的实现

# subsection 原始数据集上训练的CART决策树模型在训练集和测试集中分错的样本比例
...
# 获取最佳模型
cart_classifier = grid_search.best_estimator_
train_label_pred = cart_classifier.predict(X_train)
test_label_pred = cart_classifier.predict(X_test)
...
# 训练样本中被CART决策树错误分类的样本
wrong_classified_train_indices = np.where(y_train != train_label_pred)[0]
print("训练样本中被CART决策树错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))

# 测试样本中被CART决策树错误分类的样本
wrong_classified_test_indices = np.where(y_test != test_label_pred)[0]
print("测试样本中被CART决策树错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))

# 整体数据集D中被CART决策树错误分类的样本
print("完整数据集D中被CART决策树错误分类的样本占总完整数据的比例：",
      (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))/(len(y_train) + len(y_test)))

# subsection 加噪数据集上训练的CART决策树在训练集和测试集中分错的样本比例

print("*" * 100)

# 使用网格搜索进行超参数优化
grid_search_noise = GridSearchCV(DecisionTreeClassifier(random_state=42), params, cv=5, scoring='accuracy')
grid_search_noise.fit(X_train_copy, y_train)

# 获取最佳模型
cart_classifier_noise = grid_search_noise.best_estimator_
train_label_pred_noise = cart_classifier_noise.predict(X_train_copy)
test_label_pred_noise = cart_classifier_noise.predict(X_test_copy)

# cart_classifier_noise = DecisionTreeClassifier()
# cart_classifier_noise.fit(X_train_copy, y_train)
# train_label_pred_noise = cart_classifier_noise.predict(X_train_copy)
# test_label_pred_noise = cart_classifier_noise.predict(X_test_copy)

# 加噪训练样本中被CART决策树错误分类的样本
wrong_classified_train_indices_noise = np.where(y_train != train_label_pred_noise)[0]
print("加噪训练样本中被CART决策树错误分类的样本占总加噪训练样本的比例：", len(wrong_classified_train_indices_noise)/len(y_train))

# 加噪测试样本中被CART决策树错误分类的样本
wrong_classified_test_indices_noise = np.where(y_test != test_label_pred_noise)[0]
print("加噪测试样本中被CART决策树错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices_noise)/len(y_test))

# 整体加噪数据集D中被CART决策树错误分类的样本
print("完整数据集D中被CART决策树错误分类的样本占总完整数据的比例：",
...
# 在加噪数据集D'上训练的CART决策树，其分类错误的样本在原含噪数据D'中的索引
...
#
# # subsection 重新在修复后的数据上训练CART决策树
#
# # 使用网格搜索进行超参数优化
# grid_search_repair = GridSearchCV(DecisionTreeClassifier(random_state=42), params, cv=5, scoring='accuracy')
# grid_search_repair.fit(X_train_copy, y_train)
#
# # 获取最佳模型
# cart_classifier_repair = grid_search_repair.best_estimator_
# y_train_pred = cart_classifier_repair.predict(X_train_copy)
# y_test_pred = cart_classifier_repair.predict(X_test_copy)
#
# # cart_classifier_repair = DecisionTreeClassifier()
# # cart_classifier_repair.fit(X_train, y_train)
# # y_train_pred = cart_classifier_repair.predict(X_train_copy)
# # y_test_pred = cart_classifier_repair.predict(X_test_copy)
#
# print("*" * 100)
...
# print("加噪标签修复后，训练样本中被CART决策树错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))
#
# # 测试样本中被CART决策树错误分类的样本
# wrong_classified_test_indices = np.where(y_test != y_test_pred)[0]
# print("加噪标签修复后，测试样本中被CART决策树错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))
#
# # 整体数据集D中被CART决策树错误分类的样本
# print("加噪标签修复后，完整数据集D中被CART决策树错误分类的样本占总完整数据的比例：",
#       (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))/(len(y_train) + len(y_test)))
...
predict_fn = lambda x: cart_classifier.predict_proba(x)
...
# subsection 重新在修复后的数据上训练CART决策树模型
...
# cart_classifier_repair = DecisionTreeClassifier()
# cart_classifier_repair.fit(X_train_copy, y_train)
# y_train_pred = cart_classifier_repair.predict(X_train_copy)
# y_test_pred = cart_classifier_repair.predict(X_test_copy)

# 训练样本中被CART决策树错误分类的样本
wrong_classified_train_indices = np.where(y_train != y_train_pred)[0]
print("加噪特征修复后，训练样本中被CART决策树错误分类的样本占总训练样本的比例：", len(wrong_classified_train_indices)/len(y_train))

# 测试样本中被CART决策树错误分类的样本
wrong_classified_test_indices = np.where(y_test != y_test_pred)[0]
print("加噪特征修复后，测试样本中被CART决策树错误分类的样本占总测试样本的比例：", len(wrong_classified_test_indices)/len(y_test))

# 整体数据集D中被CART决策树错误分类的样本
print("加噪特征修复后，完整数据集D中被CART决策树错误分类的样本占总完整数据的比例：",
      (len(wrong_classified_train_indices) + len(wrong_classified_test_indices))/(len(y_train) + len(y_test)))