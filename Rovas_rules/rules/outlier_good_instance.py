"""
𝑅(𝑡) ∧ outlier(𝐷, 𝑅, 𝑡 .𝐴, 𝜃 ) ∧ loss(M, D, 𝑡) ≤ 𝜆 → good(𝑡)的具体实现
"""
...
# 在训练数据上的好的异常值索引下标
good_samples = np.where(hinge_loss <= 1)[0]
# print("损失函数在阈值之下的样本索引为：", good_samples)

# section 确定所选属性A下元组t是否为good outliers，确定属性A下的所有good outliers
# 计算谓词outlier(𝐷, 𝑅, 𝑡 .𝐴, 𝜃 )为true和loss(M, D, 𝑡) ≤ 𝜆的元组的交集
intersection = np.intersect1d(outliers_index_numpy, train_indices[good_samples])

if np.isin(row_indices, intersection):
    print("所选元组t在属性A下是good outliers")
else:
    print("所选元组t在属性A下不是good outliers")

print("A属性下所有的good outliers的行索引为：", intersection)

# section 确定所有属性下的所有good outliers
...
for column in range(column_num):
    ...
    outliers_index_numpy = np.array(outliers_index)
    intersection = np.intersect1d(outliers_index_numpy, train_indices[good_samples])
    good_outlier_list[column] = intersection

for idx, key in enumerate(good_outlier_list):
    value = good_outlier_list[key]
    print(f"第 {key} 列的good outliers为: {value}")