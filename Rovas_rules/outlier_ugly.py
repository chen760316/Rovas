...
# # SUBSECTION LIME(Local Interpretable Model-Agnostic Explanation), 通过扰动输入样本（perturb the input），来对模型的预测结果进行解释，需要借助XGBoost
# 所选的进行LIME分析的样本
# i = 16
# top_k_LIME = 6
# np.random.seed(1)
...
# # SUBSECTION 借助SHAP(Shapley Additive explanation)值得到有影响力的特征，需要借助XGBoost（维度不匹配报错）
# # 所选的进行SHAP分析的样本
# i = 16
# top_k_shap = 6
...
# print("字面实现方法找到的符合条件的元组数量为：", len(satisfying_indices))
