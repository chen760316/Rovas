...
class GOAD(BaseDeepAD):
    ...
    def training_prepare(self, X, y):
        ...
        dataset = TensorDataset(x_trans, labels)  # 生成数据集
        ...
        ).to(self.device)  # 构造GoadNet网络
        weights_init(net)  # 初始化网络权重

        criterion = GoadLoss(alpha=self.alpha, margin=self.margin, device=self.device)

        if self.verbose >= 2:
            print(net)  # 选择详细模式则打印net

        return train_loader, net, criterion  # 返回数据集加载器，GoadNet网络和GoadLoss损失函数
    ...
    def inference_forward(self, batch_x, net, criterion):
        ...
        s = s.sum(1)
        return batch_rep, s  # 对输入数据进行前向传播，并计算出表示batch_rep以及通过差异计算得到的s值
